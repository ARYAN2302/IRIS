#!/usr/bin/env python3
"""
IRIS v11 — LeJEPA + SIGReg + Hierarchical SupCon

v10 problem: Standard SupCon treats every drone type as a separate class.
Same-type drones are positives, different-type drones are NEGATIVES.
This creates 30 isolated type clusters with no shared "drone-ness" structure.
Holdout drone types land in unexplored regions and look indistinguishable from BGs.

v11 fix: Hierarchical Contrastive Learning (Salesforce, CVPR 2022).
  "Use All The Labels: A Hierarchical Multi-Label Contrastive Learning Framework"

Label hierarchy:
  - Parent level: "drone" (labels 0-29) vs "background" (label 30)
  - Child level: individual drone types (0-29)

Loss decomposition:
  L_HierSupCon = w_fine * L_fine + w_coarse * L_coarse

  L_fine:   Standard SupCon at type level — same-type = strong positive
  L_coarse: SupCon at superclass level — all drones = weak positive

This forces the encoder to learn BOTH drone-ness AND type identity.
The 30 clusters merge into one "drone" region with type sub-clusters.

Usage:
  modal run scripts/train_modal_v11.py
"""
...
app = modal.App("iris-v11-hierarchical-supcon")
...
CFG = dict(
    ...
    # Loss weights
    lam_sig=1e-3,
    alpha_supcon=0.05,          # total hierarchical SupCon weight (same as v10)
    supcon_temperature=0.07,

    # Hierarchical SupCon weights
    w_fine=0.7,                 # weight for fine-grained (type-level) SupCon
    w_coarse=0.3,               # weight for coarse-grained (drone-ness) SupCon
...
def supcon_loss_core(embeddings, pos_mask, temperature=0.07):
    """
    Core SupCon loss computation with a custom positive mask.

    Args:
        embeddings: (B, D) encoder embeddings
        pos_mask: (B, B) boolean mask — True for positive pairs
...
def hierarchical_supcon_loss(embeddings, labels, n_drone_types, temperature=0.07,
                              w_fine=0.7, w_coarse=0.3):
    """
    Hierarchical Supervised Contrastive Loss (Salesforce CVPR 2022).

    Label hierarchy:
      - Fine-grained (type-level): labels 0..29 are individual drone types,
        label 30 is background. Same-type = positive.
      - Coarse-grained (superclass-level): labels 0..29 → superclass "drone" (0),
        label 30 → superclass "background" (1). All drone types = positive.

    L = w_fine * L_fine + w_coarse * L_coarse

    L_fine:   Type-level SupCon. Same drone type = strong positive.
              Different drone type = negative. Background = negative.
    L_coarse: Superclass-level SupCon. All drone types = weak positive.
              Background = negative.
    """
    device = embeddings.device
    B = embeddings.shape[0]

    # ── Fine-grained positive mask (same type) ──
    fine_pos_mask = labels.unsqueeze(0) == labels.unsqueeze(1)  # (B, B)
    # Remove self-pairs
    diag_mask = ~torch.eye(B, dtype=torch.bool, device=device)
    fine_pos_mask = fine_pos_mask & diag_mask

    # ── Coarse-grained positive mask (same superclass) ──
    # Superclass: drone (labels 0..n_drone_types-1) → 0, background (label n_drone_types) → 1
    super_labels = (labels >= n_drone_types).long()  # 0 = drone, 1 = bg
    coarse_pos_mask = super_labels.unsqueeze(0) == super_labels.unsqueeze(1)  # (B, B)
    coarse_pos_mask = coarse_pos_mask & diag_mask

    # ── Compute both losses ──
    L_fine = supcon_loss_core(embeddings, fine_pos_mask, temperature=temperature)
    L_coarse = supcon_loss_core(embeddings, coarse_pos_mask, temperature=temperature)

    # ── Weighted combination ──
    loss = w_fine * L_fine + w_coarse * L_coarse
...
class IRISDroneMatchedBGDataset(Dataset):
    """
    v11 Training dataset: Drones + MATCHED backgrounds.
    Same as v10 — returns fine-grained labels (0..29 for types, 30 for BG).
    The hierarchical loss handles the grouping internally.
    """
...
class LeJEPASupConV11(nn.Module):
    """
    v11: LeJEPA + SIGReg + Hierarchical SupCon.

    The only change from v10 is in compute_loss:
      - v10: supcon_loss(z_all, labels_all)  — flat, 31 independent classes
      - v11: hierarchical_supcon_loss(z_all, labels_all, n_drone_types)  —
             fine-grained (type-level) + coarse-grained (drone-ness)
    """
    def __init__(self, cfg, n_drone_types=30):
        super().__init__()
        self.n_drone_types = n_drone_types
    ...
    def compute_loss(self, z1, z2, p1, p2, q1, q2, labels,
        ...
        return total_loss, {
            ...
            "fine_loss": sc_metrics["fine_loss"],
            "coarse_loss": sc_metrics["coarse_loss"],
...
def evaluate_with_detection(encoder, h5_path, matched_path, device="cuda"):
    """
    v11 evaluation: Same as v10 — measures random BG AUC + matched BG AUC.
    """
    ...
    # ── Cross-type silhouette: measure how close different drone types are ──
    # This is the KEY metric for v11 — if hierarchical loss works, drone types
    # should be closer together (lower silhouette = more merged clusters)
    if len(np.unique(train_labels[train_labels < n_types])) > 1:
        drone_mask = train_labels < n_types
        if drone_mask.sum() > 10:
            sil_drone_only = silhouette_score(
                train_embs[drone_mask], train_labels[drone_mask], metric="cosine")
            results["silhouette_drone_only"] = float(sil_drone_only)
...
def train():
    ...
    # ── Data first (need n_drone_types for model) ──
    ...
    n_drone_types = train_ds.n_drone_types

    model = LeJEPASupConV11(cfg, n_drone_types=n_drone_types).to(device)
    ...
    print(f"\n{'='*70}")
    print(f"IRIS v11 — HIERARCHICAL SupCon (Salesforce CVPR 2022)")
    print(f"{'='*70}")
    print(f"Dataset: {len(train_ds)} samples "
          f"({n_drone_types} drone types + matched bg)")
    ...
    print(f"w_fine={cfg['w_fine']}, w_coarse={cfg['w_coarse']}")
    print(f"  Fine:   Same-type pairs = strong positives (weight {cfg['w_fine']})")
    print(f"  Coarse: All drone types = weak positives (weight {cfg['w_coarse']})")
    ...
    for epoch in range(cfg["epochs"]):
        ...
        for batch_idx, (view1, view2, label) in enumerate(dl):
            ...
            for k in ["align_loss", "sig_loss", "supcon_loss", "fine_loss", "coarse_loss"]:
            ...
            if (batch_idx + 1) % (50 * grad_accum) == 0:
                ...
                print(
                    ...
                    f"fine={metrics['fine_loss']:.4f} coarse={metrics['coarse_loss']:.4f} "
        ...
        avg_fine = np.mean(epoch_metrics["fine_loss"])
        avg_coarse = np.mean(epoch_metrics["coarse_loss"])
        ...
        ckpt_path = f"/models/lejepa_v11_epoch{epoch}.pt"
        torch.save({
            "epoch": epoch,
        ...
        if (epoch + 1) % cfg["eval_every"] == 0:
            ...
            sil_drone = eval_results.get("silhouette_drone_only", 0)
            ...
            print(f"  Silhouette (all): {sil:.3f}")
            print(f"  Silhouette (drone-only): {sil_drone:.3f}")
            ...
            # Lower drone-only silhouette = clusters merging = hierarchical loss working
            if sil_drone > 0:
                print(f"  [Hierarchical] Drone-only silhouette: {sil_drone:.3f} "
                      f"(lower = more merged clusters)")

            # Combined metric: weight matched_auc 2x, reward lower drone silhouette (merged clusters)
            ...
            if combined > best_combined:
                ...
                best_epoch = epoch
                patience_counter = 0
                best_path = "/models/lejepa_v11_best.pt"
                torch.save({
                    "epoch": epoch,
                    "model": model.state_dict(),
                    "cfg": cfg,
                ...
                print(f"  ★ New best! Combined={combined:.4f} "
                      f"(Sil={sil:.3f}, Sil_drone={sil_drone:.3f}, Random={random_auc:.3f}, "
            ...
            eval_path = f"/models/lejepa_v11_epoch{epoch}_eval.json"
