"""Train encoder on pre-computed Zenodo SCF samples; evaluate on DRFF-R2 holdout.

Strategy:
  - Training data: 6000 pre-computed Zenodo SCF images (real RF, no augmentation)
    + BG (background, non-drone) from iris_matched_bg.h5 — labeled drone=0
  - Holdout test: DRFF-R2 (8 drone types, 1000 samples) — fully OOD
    - DJI Mavic 3C/3S/3, Mavic Air 2/2s, Mini 3 Pro/4 Pro/5 Pro
    - All DJI drones, but DIFFERENT models than Zenodo's set
    - This tests whether the encoder generalizes to unseen drone types

Output:
  /models/rf_scf_real_encoder.pt       — trained encoder
  /results/rf_scf_real_eval.json       — full eval metrics
  /results/rf_scf_real_drffr2.json     — per-drone-type breakdown
"""
import modal, os

app = modal.App("iris-cuas-rf-scf-real")
...
# The training script as a string (will be mounted into container)
TRAIN_CORE = r'''
"""Train encoder on pre-computed Zenodo SCF; evaluate on DRFF-R2 holdout."""
...
# ============ Model components (same as original train_rf_scf_core) ============
...
# ============ SCF computation (same as original) ============
...
def main(seed=42, n_epochs=30, n_bg=600, bg_eval_n=300):
    ...
    print("="*70, flush=True)
    print(f"=== RF SCF REAL Training (seed={seed}, epochs={n_epochs}) ===", flush=True)
    print("="*70, flush=True)

    ZENODO_H5 = "/data/zenodo_scf_samples_v2.h5"
    DRFFR2_H5 = "/data/data/drffr2.h5"
    BG_H5 = "/data/data/iris_matched_bg.h5"

    # 1. Load pre-computed Zenodo SCF samples
    print("\n[1] Loading pre-computed Zenodo SCF samples...", flush=True)
    with h5py.File(ZENODO_H5, "r") as f:
        zenodo_scf = f["images"][:].astype(np.float32)   # (6000, 2, 256, 256)
        zenodo_sources = [s.decode() for s in f["sources"][:]]
        zenodo_types = [t.decode() for t in f["types"][:]]
    n_zenodo = len(zenodo_scf)
    print(f"  Loaded {n_zenodo} Zenodo SCF images", flush=True)
    print(f"  Unique source files: {len(set(zenodo_sources))}", flush=True)
    print(f"  Unique drone types:  {len(set(zenodo_types))}", flush=True)

    # 2. Load BG samples (background, non-drone)
    print(f"\n[2] Loading BG samples (target: {n_bg}+{bg_eval_n})...", flush=True)
    with h5py.File(BG_H5, "r") as f:
        ...
        bg_specs_train = np.stack([
            bg_grp[k][:][:2].copy().astype(np.float32) for k in bg_train_keys
        ])
        bg_specs_eval = np.stack([
            bg_grp[k][:][:2].copy().astype(np.float32) for k in bg_eval_keys
        ])
    # BG may have different shape — resize to (2,256,256)
    ...
    print(f"  BG shape: {bg_specs_train.shape}", flush=True)

    # 3. Compute DRFF-R2 SCF (holdout test set)
    print(f"\n[3] Computing DRFF-R2 SCF (holdout test)...", flush=True)
    drffr2_scf = {}
    with h5py.File(DRFFR2_H5, "r") as f:
        ...
        for tname in sorted(drones.keys()):
            ...
            for sk in sorted(type_grp.keys()):
                raw = type_grp[sk][:]
                # DRFF-R2 format: (3, 65536) — ch0=I, ch1=Q (both in [0,1])
            ...
            if specs:
                ...
                print(f"    {tname}: {len(specs)} SCF images", flush=True)
    n_drffr2 = sum(len(v) for v in drffr2_scf.values())
    print(f"  DRFF-R2 total: {n_drffr2} SCF images across {len(drffr2_scf)} drone types", flush=True)

    # 4. Combine training data
    ...
    encoder.eval()
    with torch.no_grad():
        # Fit Mahalanobis on training drone embeddings (Zenodo only)
        td_embs = encode(encoder, zenodo_scf, device)
        centroid, cov_inv = fit_mahal(td_embs)
        td_dists = mahal_l2(td_embs, centroid, cov_inv)
        # Threshold = 99th percentile of training drone distances
        thresh = float(np.percentile(td_dists, 99))
        print(f"  Threshold (99p of train drones): {thresh:.2f}", flush=True)
        print(f"  Train drone dist: mean={td_dists.mean():.2f} median={np.median(td_dists):.2f}",
              flush=True)

        # BG false positive rate
        be_embs = encode(encoder, bg_specs_eval, device)
        be_dists = mahal_l2(be_embs, centroid, cov_inv)
        bg_fp = float((be_dists <= thresh).mean())
        print(f"  BG eval: FP={bg_fp:.4f}  dist_mean={be_dists.mean():.2f}", flush=True)

        # DRFF-R2 per-drone-type detection
        print(f"\n  DRFF-R2 holdout detection (per drone type):", flush=True)
        per_type_results = {}
        all_drffr2_dists = []
        ...
        print(f"\n  DRFF-R2 overall: det={drffr2_det:.4f}  dist_mean={all_drffr2.mean():.2f}",
              flush=True)

        # AUC for drone vs BG discrimination
        all_bg_plus_drffr2_dists = np.concatenate([be_dists, all_drffr2])
        all_labels_auc = np.concatenate([np.zeros(len(be_dists)), np.ones(len(all_drffr2))])
        try:
            auc = float(roc_auc_score(all_labels_auc, -all_bg_plus_drffr2_dists))
        except Exception:
            auc = -1.0
        ...
        # Source probe: logistic regression on Zenodo (label 0) vs DRFF-R2 (label 1)
        # If high → encoder learns source-specific features (bad for generalization)
        # If ~50% → encoder doesn't distinguish sources (good — domain-invariant)
        ...
        try:
            ...
            probe = -1.0
        print(f"  Source probe (Zenodo vs DRFF-R2): {probe:.4f}  "
              f"(~0.5 = good domain invariance)", flush=True)
    ...
    output = {
        ...
        "method": "RF SCF trained on real Zenodo SCF (no augmentation), eval on DRFF-R2",
        "seed": seed,
        "n_epochs": n_epochs,
        ...
        "n_drffr2_types": int(len(drffr2_scf)),
        "threshold": thresh,
        "train_drone_dist_mean": float(td_dists.mean()),
        "bg_fp": bg_fp,
        "bg_dist_mean": float(be_dists.mean()),
        "drffr2_det_overall": drffr2_det,
        "drffr2_dist_mean": float(all_drffr2.mean()),
    ...
    print(f"  BG false positive rate: {bg_fp:.4f}  (target: <0.01)", flush=True)
    print(f"  DRFF-R2 detection:      {drffr2_det:.4f}  (target: >0.50)", flush=True)
    print(f"  AUC (BG vs DRFF-R2):    {auc:.4f}  (target: >0.90)", flush=True)
    print(f"  Source probe:           {probe:.4f}  (target: ~0.5 = domain-invariant)",
          flush=True)
    print(f"  Effective dim:          {cm['eff_dim']:.1f}  (target: >10)", flush=True)
    valid = (bg_fp < 0.05 and drffr2_det > 0.30 and cm['eff_dim'] > 5)
    print(f"  VALID: {'YES' if valid else 'NO'}", flush=True)

    return output
'''

# Write the training core to a temp file and add to image
CORE_PATH = "/tmp/train_rf_scf_real_core.py"
with open(CORE_PATH, "w") as f:
    f.write(TRAIN_CORE)

IMAGE = IMAGE.add_local_file(CORE_PATH, "/root/train_rf_scf_real_core.py")
...
def launch(seed=42, n_epochs=30):
    ...
    from train_rf_scf_real_core import main
